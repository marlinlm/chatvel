...
UPLOAD_ROOT_PATH = os.path.join(root_path, ".db", "content")
print("LOCAL DATA PATH:", UPLOAD_ROOT_PATH)
# 如果不存在则创建
if not os.path.exists(UPLOAD_ROOT_PATH):
    os.makedirs(UPLOAD_ROOT_PATH)
PDF_MODEL_PATH = os.path.join(root_path, ".models/pdf_to_markdown")

nltk_data_path = os.path.join(root_path, '.data/nltk_data')

# 将你的本地路径添加到nltk的数据路径中
nltk.data.path.append(nltk_data_path)

OCR_MODEL_PATH = os.path.join(root_path, "ocr", "ocr_models")
...
SQLITE_DATABASE = os.path.join(root_path, ".db", "chatvel.db")
MILVUS_LITE_LOCATION = os.path.join(root_path, ".db", "milvus")
FAISS_LOCATION = os.path.join(root_path, ".db", "faiss")
...
LOCAL_RERANK_PATH = os.path.join(root_path, '.models/rerank', 'rerank_model_configs_v0.0.1')
if not os.path.exists(LOCAL_RERANK_PATH):
    os.makedirs(LOCAL_RERANK_PATH)
if os_system == 'Darwin':
...
LOCAL_EMBED_PATH = os.path.join(root_path, '.models/embedding', 'bge-base-zh-v1.5')
if not os.path.exists(LOCAL_EMBED_PATH):
    os.makedirs(LOCAL_EMBED_PATH)
if os_system == 'Darwin':
    ...
    LOCAL_EMBED_REPO = "AI-ModelScope/bge-base-zh-v1.5"
    LOCAL_EMBED_MODEL_PATH = os.path.join(LOCAL_EMBED_PATH, "pytorch_model.bin")
print('LOCAL_EMBED_REPO:', LOCAL_EMBED_REPO)
LOCAL_EMBED_MODEL_NAME = 'embed'
LOCAL_EMBED_MAX_LENGTH = 512

POS_PATH = os.path.join(root_path, '.models/pos','nlp_structbert_part-of-speech_chinese-base')
if not os.path.exists(POS_PATH):
    os.makedirs(POS_PATH, exist_ok=True)
POS_MODEL_PATH = os.path.join(POS_PATH, 'pytorch_model.bin')
POS_MODEL_REPO = 'damo/nlp_structbert_part-of-speech_chinese-base'

# VLLM PARAMS
model_path = os.path.join(root_path, ".models", "vllm" ,"custom_models")
# 检查目录是否存在，如果不存在则创建
