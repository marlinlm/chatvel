...
from ocr.ocr import OCRQAnything
...
class LocalDocQA:
    ...
    def insert_files_to_faiss(self, user_id, kb_id, local_files: List[LocalFile]):
        ...
        for local_file in local_files:
            ...
            add_ids = async_run(self.faiss_client.add_document(local_file.docs))
    ...
    def save_xhs_files_to_local(self, dir, local_files: List[LocalFile]):
        debug_logger.info(f'save_files_to_local: {dir}')
        success_list = []
        failed_list = []
        ...
        for local_file in local_files:
            start = time.time()
            try:
                if not local_file.file_path.lower().endswith('xhs'):
                    debug_logger.warning('skipped non xhs doc:{}'.format(local_file.file_path))
                    continue
                local_file.split_file_to_docs(self.get_ocr_result, self.retrieve_poi, self.llm)
            except Exception as e:
                error_info = f'split error: {traceback.format_exc()}'
                debug_logger.error(error_info)
                self.mysql_client.update_file_status(local_file.file_id, status='red', reason='split或embedding失败，请检查文件类型，仅支持[md,txt,pdf,jpg,png,jpeg,docx,xlsx,pptx,eml,csv]')
            ...
            # for doc in local_file.docs:
                ...
                # if 'pois' in doc.metadata and doc.metadata['pois']:
                #     pois = doc.metadata['pois']
                #     doc.metadata['poi_retrieved'] = {}
                    # for poi in pois:
                    #     searched_pois = self.retrieve_poi(poi)
                    #     if searched_pois[0].metadata['score'] < 0.38:
                    #         doc.metadata['poi_retrieved'][poi]=' '.join([searched_pois[0].page_content, str(searched_pois[0].metadata['score'])])
                    #     else:
                    #         doc.metadata['poi_retrieved'][poi]=None
                            
                # poi_dir = os.path.join(dir, 'poi')
                # os.makedirs(poi_dir, exist_ok=True)
                # file_name = doc.metadata['id']+ '.poi'
                # file_path = os.path.join(poi_dir, file_name)
                # with open(file_path, "wb+") as f:
                #     if 'poi_retrieved' in doc.metadata and doc.metadata['poi_retrieved']:
                #         f.write(str(doc.metadata['poi_retrieved']).encode('utf-8'))
                #     else:
                #         f.write('none'.encode('utf-8'))
    ...
    def retrieve_poi(self, query, kb_ids = ['beijing_poi'], need_web_search=False, score_threshold=0.35):
        retrieval_documents = self.local_doc_search(query, kb_ids,merge=False)
        if need_web_search:
            retrieval_documents.extend(self.web_page_search(query, top_k=3))
        ...
        retrieval_documents = sorted(merged.values(), key=lambda x: x.metadata['score'], reverse=False)
