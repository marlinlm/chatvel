...
from config.model_config import UPLOAD_ROOT_PATH, SENTENCE_SIZE, ZH_TITLE_ENHANCE, USE_FAST_PDF_PARSER, PDF_MODEL_PATH
from langchain.docstore.document import Document
from loader.my_recursive_url_loader import MyRecursiveUrlLoader
...
from splitter import ChineseTextSplitter,zh_title_enhance
from loader import UnstructuredPaddleImageLoader, UnstructuredPaddlePDFLoader, UnstructuredPaddleAudioLoader
from loader.markdown_parser import convert_markdown_to_langchaindoc
...
class LocalFile:
    def __init__(self, user_id, kb_id, file: Union[File, str, dict], file_id, file_name, embedding, is_url=False, in_milvus=False, headers=None):
        ...
        if is_url:
            ...
            self.file_path = "URL"
            self.file_content = b''
        elif file_name.endswith('poi'):
            self.file_path = "POI"
            self.file_content = file
        elif file_name.endswith('xhs'):
            self.file_path = file
    ...
    @get_time
    def split_file_to_docs(self, ocr_engine: Callable, retrieve_poi: Callable = None, llm:object = None, sentence_size=SENTENCE_SIZE,
                           using_zh_title_enhance=ZH_TITLE_ENHANCE):
        if self.file_path.lower().endswith('xhs'):
            debug_logger.info("load xhs: {}".format(self.file_name))
            loader = XhsLoader(ocr_engine=ocr_engine, poi_retriever=retrieve_poi, llm = llm, file_path=self.file_path, headers=self.headers, save_raw=True, save_parsed=True, save_poi_extraction=True)
            docs = loader.load()
        elif self.url:
            debug_logger.info("load url: {}".format(self.url))
            loader = MyRecursiveUrlLoader(url=self.url, headers=self.headers)
            textsplitter = ChineseTextSplitter(pdf=False, sentence_size=sentence_size)
            docs = loader.load_and_split(text_splitter=textsplitter)
        elif self.file_path == 'POI':
            docs = []
            slices = self.file_content['content'].split(';')
            for s in slices:
                docs.append(Document(page_content=s, metadata=self.file_content['meta']))
            ...
            if USE_FAST_PDF_PARSER:
                ...
                try:
                    from loader.self_pdf_loader import PdfLoader
        ...
        if not self.file_path.lower().endswith(".csv") and not self.file_path.lower().endswith(".xlsx") and not self.file_path == 'FAQ' and not self.file_path.lower().endswith(".xhs") and not self.file_path == 'POI':
            new_docs = []
            min_length = 200
            for doc in docs:
                if not new_docs:
